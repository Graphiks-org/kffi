...
class X11BindingTest {
    ...
    fun generatedRecordAccessorsRoundTripNativeFields() {
        ...
        arena.use {
            ...
            XImage.data(image, pixels)
            XImage.bytes_per_line(image, 64)
            XImage.bits_per_pixel(image, 32)
            assertEquals(pixels, XImage.data(image))
            assertEquals(64, XImage.bytes_per_line(image))
            assertEquals(32, XImage.bits_per_pixel(image))

            val attributes = XWindowAttributes.allocate(it)
            XWindowAttributes.map_state(attributes, IsViewable())
            assertEquals(IsViewable(), XWindowAttributes.map_state(attributes))

            val clientMessage = XClientMessageEvent.allocate(it)
            XClientMessageEvent.type(clientMessage, 33)
            XClientMessageEvent.window(clientMessage, 0x10203040L)
            XClientMessageEvent.message_type(clientMessage, 0x50607080L)
            XClientMessageEvent.format(clientMessage, 32)
            XClientMessageEvent.data_l0(clientMessage, 0x1122334455667788L)
            assertEquals(33, XClientMessageEvent.type(clientMessage))
            assertEquals(0x10203040L, XClientMessageEvent.window(clientMessage))
            assertEquals(0x50607080L, XClientMessageEvent.message_type(clientMessage))
            assertEquals(32, XClientMessageEvent.format(clientMessage))
            assertEquals(0x1122334455667788L, XClientMessageEvent.data_l0(clientMessage))

            val selection = XSelectionEvent.allocate(it)
            XSelectionEvent.requestor(selection, 0x1111L)
            XSelectionEvent.selection(selection, 0x2222L)
            XSelectionEvent.target(selection, 0x3333L)
            XSelectionEvent.property(selection, 0x4444L)
            assertEquals(0x1111L, XSelectionEvent.requestor(selection))
            assertEquals(0x2222L, XSelectionEvent.selection(selection))
            assertEquals(0x3333L, XSelectionEvent.target(selection))
            assertEquals(0x4444L, XSelectionEvent.property(selection))

            val windowAttributes = XSetWindowAttributes.allocate(it)
            XSetWindowAttributes.override_redirect(windowAttributes, 1)
            assertEquals(1, XSetWindowAttributes.override_redirect(windowAttributes))
        }
    }
    ...
    fun generatedRecordLayoutsMatchLinuxX11Abi() {
        ...
        assertEquals(160L, XWindowAttributes.layout.byteSize())
        assertEquals(116L, XWindowAttributes.layout.byteOffset(groupElement("map_state")))
