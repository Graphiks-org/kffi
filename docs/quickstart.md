...
Timestamped snapshot artifacts produced by the repository snapshot workflow are
available from the Central Portal snapshot repository:
...
version through `kffi.version` or `KFFI_VERSION`, not `1.0.0-SNAPSHOT`, and must
resolve it from the `CentralPortalSnapshots` repository shown above.
