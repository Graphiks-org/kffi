...
Release artifacts are consumed from Maven Central. Timestamped snapshot
artifacts produced by the repository snapshot workflow are available from the
Central Portal snapshot repository, so consumers of those snapshots need both
repositories:
...
`1.0.0-SNAPSHOT`. This is the Gradle default and intended standalone snapshot
coordinate after the M2.4 migration. The dependency examples use the explicit
`<published-version>` placeholder so the selected publication is not
ambiguous. For a publication from the current snapshot workflow, set
`kffi.version` or `KFFI_VERSION` to the exact `YYYYMMDDHHMMSS-SNAPSHOT` version
generated by that run; do not use `1.0.0-SNAPSHOT` for that workflow
publication. Resolve that timestamped version from the named Central Portal
snapshot repository shown above.
...
> **M2.4 note:** The default and intended standalone snapshot coordinate is
...
>
> Replace `<published-version>` in the examples below with `1.0.0-SNAPSHOT`
> when using the Gradle default, or with the exact timestamped version generated
> by the current snapshot workflow when consuming that workflow publication;
> timestamped workflow snapshots require the `CentralPortalSnapshots` repository
> above.
...
dependencies {
    implementation("org.graphiks:kffi-jvm:<published-version>")
}
```
...
dependencies {
    implementation("org.graphiks:kffi-android:<published-version>")
}
```
...
- With no `kffi.version` or `KFFI_VERSION` override, the build uses
  `1.0.0-SNAPSHOT`, the intended standalone snapshot coordinate after M2.4. Before
