...
/**
 * Kotlin/JVM wrapper for Objective-C class: GCPhysicalInputElementCollection
 * Superclass: NSObject
...
open class GCPhysicalInputElementCollection(override val ptr: MemorySegment) : NSObject(ptr) {
    companion object {
        private val _class: MemorySegment by lazy { LOOKUP.let { ObjCRuntime.getClass("GCPhysicalInputElementCollection") } }
    ...
    @PlatformAvailability(platform = "macos", introducedMajor = 13, introducedMinor = 0, introducedSubminor = -1)
    @PlatformAvailability(platform = "tvos", introducedMajor = 16, introducedMinor = 0, introducedSubminor = -1)
    open fun elementForAlias(alias: MemorySegment): MemorySegment {
        val sel = ObjCRuntime.sel("elementForAlias:")
        return ObjCRuntime.msgSend(ValueLayout.ADDRESS, ptr, sel, alias) as MemorySegment
    }
    ...
    /** @return NSEnumerator<Element> * */
    @PlatformAvailability(platform = "ios", introducedMajor = 16, introducedMinor = 0, introducedSubminor = -1)
    @PlatformAvailability(platform = "macos", introducedMajor = 13, introducedMinor = 0, introducedSubminor = -1)
    @PlatformAvailability(platform = "tvos", introducedMajor = 16, introducedMinor = 0, introducedSubminor = -1)
    open fun elementEnumerator(): MemorySegment {
        val sel = ObjCRuntime.sel("elementEnumerator")
        return ObjCRuntime.msgSend(ValueLayout.ADDRESS, ptr, sel) as MemorySegment
    }
...
fun GCPhysicalInputElementCollection.countByEnumeratingWithState_objects_count(state: NSFastEnumerationStatePointer, buffer: MemorySegment, len: Long): Long {
