...
open class NSFormatter(override val ptr: MemorySegment) : NSObject(ptr) {
    companion object {
        private val _class: MemorySegment by lazy { LOOKUP.let { ObjCRuntime.getClass("NSFormatter") } }
    ...
    open fun isPartialStringValid_proposedSelectedRange_originalString_originalSelectedRange_errorDescription(partialStringPtr: MemorySegment, proposedSelRangePtr: NSRangePointer, origString: MemorySegment, origSelRange: NSRange, error: MemorySegment): Boolean {
        val sel = ObjCRuntime.sel("isPartialStringValid:proposedSelectedRange:originalString:originalSelectedRange:errorDescription:")
        return ObjCRuntime.msgSend(ValueLayout.JAVA_BOOLEAN, ptr, sel, partialStringPtr, proposedSelRangePtr.segment, origString, ObjCRuntime.ObjCStructArg(origSelRange.segment, NSRange.layout), error) as Boolean
    }
