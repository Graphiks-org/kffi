...
class ObjCMethodRouter internal constructor(
    ...
    /**
     * Binds a `BOOL(id, SEL)` [selector] to [handler]. [fallback] is returned if the handler
     * throws after callback admission; calls without a route or after revocation return ABI zero.
     */
    fun onBoolean(
        selector: String,
        fallback: Boolean,
    ...
    internal fun booleanNoArgumentFallback(command: Long): Boolean =
...
internal object ObjCMethodDispatch {
    ...
    fun dispatchBoolean(
        ...
        val route = acquireRoute(boundary, self) ?: return boundary.fallback
        val fallback = route.router.booleanNoArgumentFallback(command)
        boundary.fallback = fallback
        beforeCallbackAdmissionForTest.get()?.invoke()
        var admitted = false
        val result = CallbackRuntime.dispatchSafely(callbackType, route.token, fallback) { callback ->
