...
`ScreenCaptureStreamConfiguration.minimumFrameInterval` is optional; when supplied, KFFI passes
its exact finite, positive Kotlin duration to `SCStreamConfiguration` as a valid nanosecond
CoreMedia time rather than silently retaining the framework default.
