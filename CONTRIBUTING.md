...
Gradle version is selected with the `kffi.version` property; the repository
snapshot workflow overrides it with `KFFI_VERSION` using the exact timestamped
`YYYYMMDDHHMMSS-SNAPSHOT` version generated by that run.
